...
using LoopIterationFn2 = function_ref<void(
    OpBuilder &builder, ValueRange memRefOperands, ValueRange loopIvs, Value alloc)>;

static void lowerOpToLoops2(Operation *op, ArrayRef<Value> operands,
                            RankedTensorType upperBounds,
                           PatternRewriter &rewriter,
                           LoopIterationFn2 processIteration) {
  auto tensorType = (*op->result_type_begin()).cast<TensorType>();
  auto loc = op->getLoc();
  ...
  buildAffineLoopNest(
      ...
      [&](OpBuilder &nestedBuilder, Location loc, ValueRange ivs) {
        // Call the processing function with the rewriter, the memref operands,
        // and the loop induction variables. This function will store the
        // generated value in alloc.
